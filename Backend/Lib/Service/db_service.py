...
from Lib.Model import DBModel, TodoModel, ConditionModelBase, TodoConditionModel
    ...
    def __exit__(self, exec_type, exec_value, exec_tb):
        ...
        if exec_tb:
            logging.getLogger("server").error(
                f"数据库关闭异常(Type: {exec_type}, Value: {exec_value}, Traceback: {exec_tb})")
...
class TodoService(DBService):
    ...
    def add(self, new_data: TodoModel) -> bool:
        ...
        try:
            ...
            logging.getLogger("server").info(
                f"添加新 Todo 成功({new_data.message})")
            return True
    ...
    def delete(self, target_data_timeID: str) -> bool:
        ...
        try:
            ...
            logging.getLogger("server").info(
                f"删除 Todo 成功({target_data_timeID})")
            return True
        except Exception as e:
    ...
    def update(self, target_data_timeID: str, new_data: TodoModel) -> bool:
        ...
        try:
            self.cursor.execute(sql, new_data.tuple()[
                                1:] + (target_data_timeID, ))
            logging.getLogger("server").info(
                f"更改 Todo 成功({target_data_timeID}, {new_data.message})")
            return True
        except Exception as e:
